...
def step_12_card_structure(cfg: WizardConfig) -> Optional[str]:
    ...
    if cfg.mission == "rag_only":
        ...
        print("  Cards will be dense claim bundles (title + entities + 3-8")
        print("  atomic bullets). No prose envelope. Not designed for human")
        print("  reading; tuned for embedding + retrieval quality.")
        print("  Example of what you will get per conversation:")
        print("")
        print("    ---")
        print("    title: \"Setting up PyTorch on M2 Mac with MPS backend\"")
        print("    entities: [PyTorch, M2 Mac, MPS, Apple Silicon, GPU]")
        print("    ---")
        print("    - PyTorch 2.0+ supports MPS backend natively")
        print("    - Use torch.device('mps') instead of cuda")
        print("    - Fallback: PYTORCH_ENABLE_MPS_FALLBACK=1")
        print("    - Install: conda install pytorch torchvision -c pytorch-nightly")
        print("    - Verify: torch.backends.mps.is_available()")
        return "SKIPPED"

    print("\n[12/16] Card structure")
    print("  A 'card' is what the refine daemon writes after each conversation.")
    print("  It lives in your Obsidian vault as Markdown AND is embedded into")
    print("  the RAG retrieval index. You can always re-refine with a different")
    print("  shape later via `python install.py --step 12 && reconfigure`.")
    print("  Three shapes ship out of the box:\n")

    print("  ---------------------------------------------------------------")
    print("  [1] Compact — title + one paragraph + tags")
    print("  ---------------------------------------------------------------")
    print("  Example card:")
    print("")
    print("    # Setting up PyTorch MPS on M2 Mac")
    print("")
    print("    PyTorch 2.0+ supports MPS natively via torch.device('mps').")
    print("    Set PYTORCH_ENABLE_MPS_FALLBACK=1 for unsupported ops. Install")
    print("    via conda install pytorch torchvision -c pytorch-nightly, then")
    print("    verify with torch.backends.mps.is_available().")
    print("")
    print("    #pytorch #m2 #mps")
    print("")
    print("  Pick if: you think in single atomic claims, want small cards,")
    print("  prefer Zettelkasten-style linking over narrative notes.")
    print("")

    print("  ---------------------------------------------------------------")
    print("  [2] Standard — 6-section skeleton (default, current v0.1 shape)")
    print("  ---------------------------------------------------------------")
    print("  Example card structure:")
    print("")
    print("    # Setting up PyTorch MPS on M2 Mac")
    print("")
    print("    # Scenario & pain point")
    print("    Why this matters. What triggered you to search it up.")
    print("")
    print("    # Core knowledge & first principles")
    print("    The underlying mental model — MPS is Apple's Metal")
    print("    Performance Shaders compute backend, cousin to CUDA.")
    print("")
    print("    # Execution — step-by-step")
    print("    1. Install via conda nightly channel.")
    print("    2. Use torch.device('mps').")
    print("    3. Flip PYTORCH_ENABLE_MPS_FALLBACK=1 if ops are missing.")
    print("")
    print("    # Avoid — pitfalls and edges")
    print("    Some ops (sparse, certain reductions) still hit CPU.")
    print("")
    print("    # Insight & mental model")
    print("    'MPS' is one more backend in torch's dispatcher; treat")
    print("    it like cuda for most intents, with a smaller op set.")
    print("")
    print("    # Summary")
    print("    MPS on M2 = near-CUDA dev ergonomics with known gaps.")
    print("")
    print("  Pick if: you want cards a future-you can read cold and")
    print("  immediately recomprehend, with narrative scaffolding.")
    print("")

    print("  ---------------------------------------------------------------")
    print("  [3] Detailed — Standard + sidebar (research-grade)")
    print("  ---------------------------------------------------------------")
    print("  Same 6 sections as Standard PLUS a trailing metadata sidebar:")
    print("")
    print("    # Setting up PyTorch MPS on M2 Mac")
    print("    [... six sections as Standard ...]")
    print("")
    print("    ---")
    print("    Related cards: M1 GPU benchmarking notes, CUDA-MPS diff summary")
    print("    Contradictions: 2023 docs said 'experimental'; 2024 says GA")
    print("    Open questions: does FAISS-GPU work on MPS yet?")
    print("    Confidence: 0.85 (multi-source, 3 independent tests)")
    print("")
    print("  Pick if: you're doing decision-making or research where")
    print("  tracking confidence / related work / open threads matters.")
    print("  Costs ~20% more per refine than Standard.")
    print("")

    print("  (After you pick, step 13 will refine ONE real conversation")
    print("   with this shape so you can see the actual result before")
    print("   committing to bulk refine.)")
