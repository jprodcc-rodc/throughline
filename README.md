...
python -m throughline_cli doctor              # 11 checks with remediation
...
| Vector store (`VECTOR_STORE`) | `qdrant` | `chroma`, `lancedb` (optional deps — embedded, zero-server) | `duckdb_vss` / `sqlite_vec` / `pgvector` |
