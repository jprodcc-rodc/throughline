...
Rodix is not built for crisis support, and pretending it is would be worse than naming the limit. If you're in distress right now: **988** (US Suicide and Crisis Lifeline), **Samaritans 116 123** (UK & Ireland), or your local equivalent — `findahelpline.com` has international numbers. The upcoming Wave 1c crisis-content protocol (ships before broader Phase 1 alpha) will surface these resources directly when explicit safety language is detected, and decline to extract a card from those messages. Until then, the resources are here, and you can also email support@rodix.app — a human reads it.
