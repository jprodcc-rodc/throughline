...
**ChatGPT memory shipped through 2024-2025 and let the user inspect a profile-of-them.** The UI surfaces saved memories — name, job, dietary preference, frequently mentioned topics, custom instructions — as discrete labels the user can view, edit, and delete in Settings; chat-history-derived insights operate alongside as a second mechanism. What ChatGPT memory does not capture, by design choice, is the *thinking* the user did across conversations: which decisions they were working through, what they'd concluded, what questions stayed open. Memory-as-profile is operationally useful for personalization (and personalization is a real use case); it is structurally unable to function as continuity-of-thought. ChatGPT did not fail to ship memory — it shipped a different kind of memory than continuity-of-thought needs.

**Gemini personalization shipped and let users correct what the system inferred.** From the friends-intro: *"Gemini decided I was a corporate secretary because I asked one factual question about the role."* What is actually shipped: tell Gemini in chat the label is wrong, and Gemini accepts the correction (verified 2026-05-04). The architectural critique is one layer down — the mechanism behind the corporate-secretary tag is profile-inference, converting conversational data into stable persona labels at some confidence threshold. Each surfaced tag is correctable; the mechanism remains profile-inference — corrections are remembered, but the underlying inference loop keeps running on new conversational data, producing new persona labels at the same confidence threshold. It is a defensible product choice for personalization, not the architecture continuity-of-thought needs.

**Claude shipped projects + cross-conversation memory and confirmed the cross-vendor pattern.** Projects are the most user-respectful memory primitive among the three vendors — each project has its own memory space and its own summary, separate from non-project chats; memory is included in Claude's data export. The structural critique narrows: not "no memory features" — Claude has them, and Claude's are arguably the most thoughtful of the three — but each vendor's memory is locked to that vendor's ecosystem by format and storage. *"Your ChatGPT memory means nothing in Claude. You change tools, you start over."* No frontier vendor has the business incentive to ship cross-vendor portability; cross-vendor portability is what dissolves switching cost. The lock-in is at the format-and-storage layer, where no single vendor will fix it.

**The structural conclusion: all three vendors made the same deliberate product choice — memory optimized for profile-of-user, not continuity-of-thought.** Profile labels (preferences, role, frequently mentioned topics) shape responses in ways that increase engagement and retention; a vault of user thinking does not improve response shaping the same way and creates portability liabilities the model-layer business cannot afford. *"If your Claude memory worked in GPT, you'd switch tools whenever a better model came out. So they keep memory locked, opaque, and uneditable — by design."* The friends-intro quote captures lock-in; underneath it sits the deeper architecture: cross-vendor portability and thinking-continuity-as-memory are not features vendors will eventually ship. They are choices vendors will not make, because they conflict with personalization-as-engagement-driver. Rodspan's bet is not that AI memory is an interesting category — every vendor has now confirmed it is. The bet is that user-aligned memory — visible cards, cross-model, real export — is structurally available only to a builder whose business model does not depend on lock-in.
