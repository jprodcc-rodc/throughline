...
**Gemini personalization shipped and let users correct what the system inferred.** From the friends-intro: *"Gemini decided I was a corporate secretary because I asked one factual question about the role."* What is actually shipped: tell Gemini in chat the label is wrong, and Gemini accepts the correction (verified 2026-05-04). The architectural critique is one layer down — the mechanism behind the corporate-secretary tag is profile-inference, converting conversational data into stable persona labels at some confidence threshold. Each surfaced tag is correctable; the same mechanism keeps producing similar tags from similar inputs because that is what profile-inference is designed to do. It is a defensible product choice for personalization, not the architecture continuity-of-thought needs.
