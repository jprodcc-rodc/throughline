...
OpenAI rolled out ChatGPT Memory in 2024 and expanded it through 2025. The result is a fully shipped feature: a settings UI where users can view, edit, and delete saved memories; a parallel chat-history-derived insights mechanism; data export as JSON. What ChatGPT Memory does, by design, is build a profile-of-user — name, job, dietary preference, frequently mentioned topics, custom instructions — that shapes future responses. What it does not do, by design, is capture the *thinking* the user did across conversations: which decisions they were working through, what they'd concluded, what questions stayed open. The friends-intro framing — *"a few items it deigns to show you, plus an iceberg of inferred labels you'll never see"* — overstated the opacity of the UI surface as of 2026-05; the deeper observation, that ChatGPT memory is profile-of-user rather than continuity-of-thought, is the real architectural critique. OpenAI shipped the kind of memory the personalization business model rewards. They did not ship the kind continuity-of-thought needs.

### 1.2 Gemini personalization shipped — and produced the "corporate secretary" failure mode

The friends-intro names the second signal with surgical specificity: *"Gemini decided I was a corporate secretary because I asked one factual question about the role."* The vendor-documented fix path is to correct the inference directly in chat; Gemini accepts the correction in real-time (verified 2026-05-04). The earlier friends-intro line — *"kept opening replies with 'as a corporate secretary, you should...' for weeks"* — overstates persistence given current vendor behavior; the architectural critique sits one layer down. The mechanism behind the corporate-secretary tag is profile-inference: conversational data converts to stable persona labels at some confidence threshold. Each surfaced tag is correctable; the same mechanism keeps producing similar tags from similar inputs because that is what profile-inference is designed to do. Profile inference is a defensible product choice for personalization. It is not user-visible structured memory of the user's own thinking, and it is not the architecture continuity-of-thought needs.

### 1.3 Claude projects shipped — and proved the same pattern across all three frontier vendors

Claude shipped projects + cross-conversation memory, confirming the cross-vendor pattern. Projects are the most user-respectful memory primitive among the three vendors — each project has its own memory space and its own dedicated summary, separate from non-project chats; memory data is included in Claude's data export. The structural critique narrows: not "no memory features" — Claude has them, and Claude's are arguably the most thoughtful of the three — but each vendor's memory is locked to that vendor's ecosystem by format and storage. Your Claude project context means nothing in GPT-5 or Gemini. The minute you decide to use a different model for a different task, you start over. As the friends-intro frames it: *"You change tools, you start over."* All three vendors made the same fundamental product choice — memory optimized for personalization within their ecosystem, profile-of-user shape. That choice aligns with their business model (lock-in plus personalization-as-engagement-driver) and is incompatible with what continuity-of-thought needs: cross-vendor portability + user-controlled thinking primitives. This is the through-line: **the market created the problem Rodspan solves; Rodspan did not invent a problem to sell into.** Incumbents shipped what they shipped, users tried it, users discovered the ceiling, and the ceiling is the opening. Rodspan's bet is not that AI memory is an interesting category — every vendor has now confirmed it is. The bet is that the user-aligned implementation is structurally available only to a builder whose business model does not depend on lock-in.
