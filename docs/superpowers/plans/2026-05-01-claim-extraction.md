...
> **Status (2026-05-02 evening, post-v1.7-ship-decision):** v1.8 — **PHASE 1 SHIP READY pending Rodc Task 15 dogfood**. Per Rodc + External Opus directive 2026-05-02 evening: ship via split-route despite 11/14 strict gate-pass because the **trust-killer signal (hallucinations) is GREEN** (2.3% across 256 field decisions in EN bucket) and the 3 failing conditions are **conservatism not fabrication** (model under-extracts on borderline cases — Vault has fewer cards than ideal but no false content). Conservatism is monitor-only; Wave 2 production telemetry will calibrate.
>
> Ship-state actions applied:
> - Split-route via env-var seam `THROUGHLINE_EXTRACTION_PROVIDER` (default "anthropic" → Haiku 4.5 via OpenRouter anthropic proxy). Operator can override to "anthropic-direct" / "auto" / "openrouter" / etc.
> - Chat path unchanged (uses active-provider model — nvidia free in dev / whatever Phase 1 setting picks)
> - Failure mode: Haiku 4.5 API fail → extractor returns ok=False → queue skips persist → polling endpoint serves null/ready → frontend silently hides Card. **Chat NEVER breaks on extraction failure.**
> - 4 new unit tests in test_extractor.py lock the env-var behavior (643 / 2 skipped passing)
>
> v1.8 ship gate (working framework):
> - **Trust killer (HARD ship blocker)**: Hallucination rate ≤ 8% per field ✅ ALL PASS ; chitchat FP ≤ 5% ✅ PASS
> - **Production accuracy (HARD ship blocker)**: Overall accuracy ≥ 75% ✅ PASS (79.3%)
> - **Conservatism (monitor only, NOT blocking)**: Per-field recall ≥ 60% — 3/4 fields under (concern 56.5 / hope 53.8 / question 55.0 vs gate 60). Wave 2 telemetry will calibrate.
> - **Per-field precision (3/4 PASS)**: topic 82 / hope 78 / question 85 ≥ 70 ; concern 56 ❌ — concern still wording-mismatch but no hallucination spike.
>
> Calibration trigger (Wave 2): if production hallucination rate > 5% sustained → emergency v4 prompt iteration. Below 5% → conservative-on-recall is acceptable for Phase 1 launch.
>
> Wave 1b code-complete pending Rodc 5-round subjective dogfood (Task 15). Dogfood ≤2/5 PASS triggers rollback to (A) hope/question relabel + v4. Dogfood ≥4/5 PASS confirms ship.
> v1.7 status preserved below for trail.
>
> **v1.7 (2026-05-02 evening, mid-iteration):** Eval rebalance + ship gate redefinition (14 conditions) + concern multi-variant relabel. v3.1 prompt + relabeled concern landed 79.3% overall PASS / 11/14 gate pass / 2.3% hallucination rate.
...
**Concern eval ground truth over-specific (v1.7 added 2026-05-02 evening; v1.8 finalizes)**: After v3.1 prompt iteration brought concern recall from 30% → 48% but the strict v1.3 gate (≥80%) remained out of reach, root-cause analysis showed the failure was eval-side, not model-side: concern ground truth was a single canonical string ("family financial stability if I quit") that human annotators would judge equivalent to paraphrases the model produces ("losing income"). LLM-judge fallback couldn't bridge this gap because the strings are lexically distant even with semantic-equivalence judging on long phrases. **Status v1.7: MITIGATED.** Two-part fix: (1) re-labeled 64 EN cases concern ground truth as `list[str]` of acceptable variants — first variant matching = TP. (2) Ship gate v1.7: per-field recall ≥60% / precision ≥70% / hallucination rate ≤8% + chitchat FP ≤5% + overall ≥75% = **14 conditions**. The hallucination cap is the real anti-trust-collapse signal (the v1.3 gate was indirect: precision encoded both hallucination and mismatch under one ratio). **Trade-off documented**: looser per-field gates accept more wording variance; stricter hallucination cap maintains trust integrity.

**Conservatism on per-field recall (v1.8 OPEN — monitor-only)**: After v1.7 relabeled eval, concern/hope/question recall still fall short of 60% gate (56.5 / 53.8 / 55.0). Per-field precision is GREEN (3/4 fields ≥70%); hallucination rate is GREEN (≤2% per field, well under 8% cap). The gap is therefore conservatism — model under-extracts on borderline cases. Production effect: Vault has fewer cards than ideal; no false content. Per Rodc + Opus directive, this is monitor-only at Phase 1 launch (NOT a ship-blocking signal). **Calibration trigger**: if Wave 2 production telemetry shows hallucination > 5% sustained, emergency v4 prompt iteration. Below 5%, conservatism is acceptable; future relabel of hope/question/topic deferred to Phase 2 prep unless dogfood signal demands it.

**Hallucination as trust killer (v1.8 MITIGATED — formalized as primary ship signal)**: The Phase 1 ship gate explicitly separates trust-killer signal (hallucination rate + chitchat FP) from completeness signal (per-field recall). Trust killer is HARD ship blocker; completeness is monitor-only. v1.7 eval result: 6 hallucinations across 256 field decisions in EN bucket (2.3%). Wave 2 telemetry confirms or invalidates this calibration on real production traffic.
