...
> **Status (2026-05-02 evening, post-rebalance):** v1.6 — Eval REBALANCE APPLIED per External Opus directive. New eval set is 64 EN / 16 ZH / 0 mixed (Phase 2 deferred); few-shot is 4 EN / 2 ZH; runner emits per-language metrics; ship gate = `metrics_by_language.en`; Chinese metrics monitored for Phase 2 prep but do NOT block. parse_4field bug also fixed (now extracts first balanced `{...}` block — previously broke when models emitted JSON-then-Rationale-prose, false-counted as 4/80 API failures in v2). Awaiting Haiku 4.5 eval verdict on rebalanced set. (v1.5 status preserved below for trail.)
>
