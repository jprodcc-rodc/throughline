# Rodspan Illustration Guide

Rodspan is anti-decoration. Most "design systems" need a long illustration guide because their products lean on visual flourish to carry warmth. Rodspan doesn't. The Card with Promise carries the warmth; the type carries the hierarchy; the amber accent carries the verification register. Illustration is a distraction from the architectural commitments.
...
The Card with Promise IS the visual identity. When Rodspan needs an image, the image is a Card with Promise rendered in the actual product. That's it.
...
- **Card-as-specimen** photography. The card is the most-photographed Rodspan object. Treat it like a museum specimen — centered, well-lit (in dark mode), no decoration around it.
- **Sticky date headers** in marketing layouts — borrowed from the Vault, used as section dividers in long-form (founder essay, HN post). The amber sticky-date register carries Rodspan visual identity into long-form text.
...
- **3D-glass effects.** Notion-AI uses these; Rodspan rejects them. Considered density, not maximalist UI.
- **Character icons / mascot art.** Rodspan has no mascot. The product is the founder's voice, not a brand character.
...
Whitespace is the Rodspan illustration. Considered density at the level of Linear / Anthropic — restraint, not maximalism. Empty states should be real empty states with one-line hints, never encouragement walls.
...
Never decorative. Never as a gradient. Never on logo / brand mark. The amber consistency is what makes the Card with Promise legible across surfaces — once a user sees an amber 1px border, they know "this is a Rodspan-verified object."
...
- Hero text uses Inter weight 200, `clamp(2rem, 5vw, 3.5rem)`. The thin weight at large size is the Rodspan visual signature.
