...
- **Specific (not abstract).** Names dates, counts, durations, exact quotes. "200 hours in." "Sept 3." "Six weeks on the same fence." Same discipline `claim_extractor.md` enforces at the data layer (4-8 word verbatim extraction).
- **Anti-spin (states own limits plainly).** Volunteers limitations no marketing copy would surface. *"Server-side recall (so we can't promise zero-knowledge — that would be a lie given the architecture)."* More active than "restrained" — quiet about achievements, willing to volunteer limits.
- **Refuses-to-dramatize (no triumph, no apology theater, no founder mythology).** *"Founder: solo, anonymous, working out of Asia, second half of a multi-year build."* Four facts. No "scrappy", no "passionate", no "on a mission to". Hardest move for an LLM to reproduce because models default to dramatizing.
...
Parentheticals qualify, soften, or undo a strong claim *without retreating from it*. The parenthetical carries the honesty — short, never decorative, used when the main clause is starker than the truth and the parenthetical brings the truth back without weakening the claim.
...
- **Anti:** *"We're committed to privacy (well, mostly — there are some edge cases of course, but we're working on them)!"* (Hedge-parenthetical drains the claim. Note the exclamation mark — friends-intro uses zero.)
...
When the friends-intro could go vague, it doesn't. User examples have dates, durations, sentence-quotes, outcomes. Competitor critiques name the company *and* the specific failure mode. Mechanism explanations name actual data ("topic / concern / hope / question") not "structured insights." Time qualifiers are concrete ("weeks, not months", "200 hours in").
...
Qualifiers like "might", "fine", "weeks, not months" don't soften — they sharpen. Honest qualifiers are short and definite. They name a specific possible counter-state, not vague reservation.
...
## 4b. Heavy-disclosure handling — Phase B-corrected note (added 2026-05-03 v1.1 → reverted v1.2)

**REVERTED:** Tier 1.5 self-simulation produced an 8th do/don't pair codifying "Marked." / "Noted." single-word acknowledgment for heavy disclosure (Daniel R6 + Emma R4). Phase B real-API verification on 2026-05-03 revealed this pattern was a CC self-simulation artifact, NOT a real Haiku 4.5 behavior. Daniel Round 6's real Haiku reply opened with *"You're right about the shape,"* — engaging the substance, not single-word-acknowledging.

**Operational lesson:** real Haiku 4.5 prefers substance-first engagement over acknowledgment-first single-word openers. Both register-options can hold brand voice (anti-spin, refuses-to-dramatize) — but the codified pattern was wrong. Trust banned-phrase discipline (which Phase B verified holds reliably) over manufactured positive patterns.

**For heavy-disclosure surfaces (grief / loss / crisis-adjacent):** the EXISTING 7 don'ts cover the failure modes. Don't #4 (no Caregiver register) + don't #2 (no adjective stacks) + don't #5 (no process verbs) cover what NOT to do. Do #4 (specific over abstract — "engage the concrete thing first") drives what TO do — engage the actual thing the user said, not the meta-acknowledgment.

**Phase B confirmed (HIGH confidence):** the explicit banned-phrase list holds against Anthropic's safety-tuning pressure across 5+ verification rounds + 2 Day-15 messages. Trust the bans. Manufactured positive patterns (like "Marked.") may not survive real-API encounter.
...
## 6. Voice consistency checklist (the ship gate)

Five yes/no questions. Two are friends-intro-pattern-specific. Walk all five before any user-facing copy ships.
...
## 8. Locale notes (English vs Chinese)
...
Chinese needs *more* discipline than English by default — the cultural pull toward sentimentality and adjective-stacking is denser.

---

## 9. Word-count ceilings

- **Hero copy:** ≤ 12 English words. Friends-intro short pitch is 8.
...
- **Status section:** bullet list of facts, ≤ 4 bullets.
