---
name: rodix-design-system
description: Use when writing any Rodix-facing copy or building Rodix UI components — covers voice (Specific. Anti-spin. Refuses-to-dramatize.), visual identity (amber #d97706 + Inter + dark-mode-default), component patterns, and microcopy. Triggered by mentions of "Rodix", "brand voice", "marketing copy", "landing page", "UI component", "design tokens", "microcopy", "Rodix product", "founder essay", "support email", "empty state", "error message", "tooltip", "vault", "card", "recall callout", "onboarding".
---
...
- Writing user-facing Rodix copy: marketing, docs, microcopy, error states, empty states, tooltips, button labels.
- Building Rodix UI components: Card with Promise, Vault list, recall callout, settings modal, onboarding screens.
- Creating Rodix marketing assets: founder essay, HN post, Twitter thread, landing page hero.
- Reviewing whether a deliverable is Rodix-coherent (voice, visual, layer routing).
- Designing onboarding / chat / vault / decisions surfaces.

## Quick reference

- **Brand archetype:** Explorer (primary) + Everyman (secondary, color only).
- **In-product AI character:** Sage-flavored Socratic (per `app/web/prompts/rodix_system.md` v1.3 — Round 1 ask, Round 2 maybe, Round 3+ stop asking and reflect).
...
- **Canonical pitch:** "ChatGPT remembers your name. Rodix remembers your thinking." (8 words, friends-intro verbatim, hero-locked.)
...
1. **NO McLuhan "we shape our tools" anchor.** Saturated by Granola + Notion. Rodix doesn't borrow it.
...
3. **NO Caregiver register:** *I'm here for you, I hear you, take your time, 慢慢说, 我陪你, that sounds really hard, whatever it is, 无论是什么.* Banned in `rodix_system.md` and extended by spirit, not just literal translation.
...
7. **NO streak counters / engagement metrics / today view / weekly summary.** Decision 7 — Rodix is for *thinking compounding*, not engagement. We never optimize for time-on-app, DAU, or message volume.
...
- `illustration-guide.md` — Minimal visual style. Rodix is anti-decoration; image use is for product UI screenshots, not marketing-illustration.

## Two-layer model — load-bearing

Rodix has two voice layers; routing the wrong one to the wrong surface is the most common failure mode.

- **Brand-as-Explorer** (sovereignty / anti-lock-in / matter-of-fact / parenthetical-as-honesty) — governs *landing pages · founder essay · HN post · Twitter thread · support emails · marketing copy · docs · onboarding · empty states · error states · tooltips · button labels*. Anywhere Rodix introduces itself or explains a system state.
- **AI-as-Sage-flavored** (Socratic / Round 1 ask / Round 2 maybe / Round 3+ stop asking and reflect) — governs *in-product chat replies* per `app/web/prompts/rodix_system.md` v1.3. Anywhere Rodix speaks AS the AI character mid-conversation.

**Routing test:** "Is this Rodix-introducing-itself, or Rodix-thinking-with-the-user?" If it explains a system state ("we couldn't extract a card"), it's Explorer. If it speaks first-person mid-conversation ("Let me ask one more thing"), it's Sage. The recall callout `⚡ 我把这个带回来了` is the AI character speaking inside chat → Sage. The vault-empty-state hint *"Your cards will appear here as you talk to Rodix. We don't write them — your thinking does."* is brand-introducing → Explorer.
...
Before any Rodix-facing copy ships, walk the 5-question checklist (`voice-guide.md` §6):
...
5. **Anti-target test.** Would the unfit user be sent away clearly, or coaxed? If the copy reads like "anyone can benefit," it's not Rodix. Rodix says: *"ChatGPT's fine."*
...
These are the lines Rodix holds. Any deliverable that violates one of these is not Rodix-coherent — even if it sounds nice.
...
- **System prompt** — `app/web/prompts/rodix_system.md` v1.3 (in-product AI character).
